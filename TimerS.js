class TimerS {
    ...
    display(fontType, fontSize) {
        ...
        if (this.left_time > 0) {
        // 남은 시간을 초 단위로 계산
            let left_seconds = Math.floor(this.left_time / 1000 +1);
            ...
            if(!this.diff_display){ // 디폴트 위치 -> 살짝 왼쪽으로 옮겨야 함
                ...
                fill(0);
                text(`TIMER  00:${display_time}`, 1468, 118);        
            } else { // 다른 위치에 그릴 때
            ...
            if(!this.diff_display){
                // 타이머가 종료되었을 때 00:00으로 표시
                text('TIMER  00:00', 1468, 118);
            } else {
                // 타이머가 종료되었을 때 00:00으로 표시
                text('TIMER  00:00', 1468, 118);
            }
